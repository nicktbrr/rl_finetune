...
class EnhancedFineTuneEnv(gym.Env):
    def __init__(self, baseline_probs, hidden_reps, true_labels, input_features=None, max_adjustment=0.6):
        super().__init__()
        ...
        if self.has_input_features:
            self.input_features = np.array(input_features)
        else:
        ...
        # Observations: [prob] + [dist_from_threshold] + [true_label] + [class_ratios] + hidden_reps + (optional) input_features
        obs_dim = 5 + self.hidden_reps.shape[1]
        if self.has_input_features:
            obs_dim += self.input_features.shape[1]
    ...
    def _get_obs(self):
        ...
        # Base observation includes prob, distance, true label, class distribution and hidden reps
        obs = np.concatenate(([prob, dist_from_threshold, float(true_label),
                              self.positive_ratio, self.negative_ratio], feats))

        # Add input features if available
        if self.has_input_features:
            input_feats = self.input_features[self.current_index]
            obs = np.concatenate((obs, input_feats))
