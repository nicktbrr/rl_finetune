...
class AccuracyCheckpointCallback(BaseCallback):
    ...
    def __init__(self,
                 ...
                 baseline_probs,
                 true_labels,
                 input_features=None,  # Add input_features parameter
                 check_freq=5000,
                 save_path='./best_model/',
                 fp_weight=1.2,  # Higher weight for false positives in the score
                 log_to_mlflow=True,
        ...
        self.baseline_probs = np.array(baseline_probs.squeeze())
        self.true_labels = np.array(true_labels)
        self.input_features = input_features  # Store input_features
        self.has_input_features = input_features is not None
        self.check_freq = check_freq
        self.save_path = save_path
        self.fp_weight = fp_weight  # Weight for false positives in score calculation
        self.log_to_mlflow = log_to_mlflow
    ...
    def _predict_batch(self, baseline_probs):
        ...
        # Use the environment's reset method to get a valid observation
        # This avoids constructing observations manually and ensures compatibility
        obs = self.eval_env.reset()[0]

        # The shape of the observation will tell us what format we need
        obs_shape = obs.shape
        ...
        positive_ratio = self.eval_env.get_attr('positive_ratio')[0]
        negative_ratio = self.eval_env.get_attr('negative_ratio')[0]
        max_adjustment = self.eval_env.get_attr('max_adjustment')[0]
        ...
        for i in range(len(baseline_probs)):
            ...
            # Create observation based on whether we have input features or not
            dist_from_threshold = abs(prob - 0.5)

            base_obs = np.concatenate(([prob, dist_from_threshold, float(true_label),
                                        positive_ratio, negative_ratio], feature))

            if self.has_input_features:
                # If we have input features, include them in the observation
                input_feat = self.input_features[i]
                obs = np.concatenate((base_obs, input_feat)).astype(np.float32)
            else:
                obs = base_obs.astype(np.float32)

            if obs.shape[0] != obs_shape[0]:
                raise ValueError(
                    f"Created observation shape {obs.shape} does not match environment observation shape {obs_shape}")
            ...
            if true_label == 0 and prob > 0.5:
                if action[0] < 0:  # Downward adjustment
                    max_downward = max_adjustment * 1.5
                    if confidence > 0.3:
                        scaled_action = action[0] * (1 + 3 * confidence**2)
                        offset = np.clip(scaled_action, -max_downward,
                                         max_adjustment * 0.5)
                    ...
                    offset = np.clip(action[0], 0, max_adjustment * 0.3)
            else:
                # Standard scaling for other cases
                if confidence > 0.3:
                    scaled_action = action[0] * (1 + 2 * confidence**2)
                    offset = np.clip(scaled_action, -max_adjustment,
                                     max_adjustment)
                else:
                    offset = np.clip(action[0], -0.2, 0.2)
