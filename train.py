...
def train_with_accuracy_checkpoints(baseline_probs, hidden_reps, true_labels,
    ...
    Args:
        ...
        hidden_reps: Hidden representations from base model
        true_labels: Ground truth labels
        input_features: Original input features (optional)
        max_adjustment: Maximum probability adjustment allowed
        timesteps: Number of training timesteps
        experiment_name: MLflow experiment name
        run_name: MLflow run name (optional)
    ...
    mlflow.log_params({
        ...
        "positive_ratio": np.mean(true_labels),
        "data_size": len(true_labels),
        "using_input_features": input_features is not None
    })

    # Create environment factory function
    def make_env():
        def _init():
            return EnhancedFineTuneEnv(
                baseline_probs,
                hidden_reps,
                true_labels,
    ...
    # Adjust network architecture based on input dimension
    hidden_layers = [256, 256, 128]
    if input_dim > 100:
        # For higher dimensional inputs, use deeper network
